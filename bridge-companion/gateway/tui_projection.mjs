...
function latestLifecycleState(snapshot, events) {
  ...
  const latestTransition = [...events].reverse().find(event => event.kind === "lifecycle_transition" || event.source === "runtime_snapshot");
  return latestTransition?.raw?.to_status || latestTransition?.messagePreview || snapshot?.run_status || "unknown";
}
