...
Expected long runtime alone is not a user-decision blocker for an already-approved formal L4 execute stage when `wait_until_process_complete` or `executor_hard_timeout_disabled` is active. Runtime-owned or direct fallback execute must not refuse to launch solely because a run may take many hours; it should launch and wait/poll in the bridge window unless concrete tool, resource, approval, or semantic evidence blocks it.
...
Minor OOMs during smoke, warmup, or formal launch are execute-owned adaptation events, not automatic bridge failures, when they can be handled by bounded changes to batch, microbatch, gradient accumulation, approved GPU selection, approved precision, or already-implemented memory-efficient toggles. Executor must preserve the resolved method/model, checkpoint, dataset, prompt/template, objective, metric, and sequence/truncation basis, log every OOM attempt and adjustment rationale, and escalate only after bounded attempts are exhausted or the next viable fix would require a method, data, objective, sequence, or resource-policy change. For the current M1 direct-LoRA seq4096 SFT case, narrow OOM at per-device batch 12 or 11 should continue through the policy-preserving ladder `10 -> 9 -> 8` before anomaly routing, preserving effective batch with gradient accumulation when possible.
...
L4 execute has strict environment and GPU rules. Formal execution uses conda env `mjy`; use `conda run -n mjy ...` or record an equivalent `conda activate mjy` context, and do not use venv/virtualenv for formal execute. Unless the user explicitly requests smoke/dry-run/conservative execution, formal GPU runs should exceed 70GB observed memory after warmup on typical 80GB GPUs, or exceed 90% of selected GPU total memory on other GPU sizes. If prelaunch free memory is below that target but a smaller approved attempt can run, executor should choose the best approved GPU, downshift batch or related memory settings, launch, and record lower usage as a deviation rather than treating the shortfall itself as a blocker.
